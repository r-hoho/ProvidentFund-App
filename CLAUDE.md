...
| `Analytics.gs` | GA4 server-side adoption metrics (Measurement Protocol). `trackEvent(name, params)` → POSTs to `/mp/collect`; `trackAppOpen(deviceData)` (visits/returning; fired **client-side** from `JS.html` DOMContentLoaded, NOT `doGet()`, so it can carry device — `doGet()` can't see the browser UA) + `trackFeatureAction(feature, outcome, deviceData)` (success/fail per action, in the 5 handlers). **Device dimensions:** `parseDevice_(ua)` buckets `navigator.userAgent` (already passed to the action handlers for the audit log) into low-cardinality `device_category`/`device_os`/`device_browser` params — never raw UA. Native GA4 device detection is useless here (hits go via UrlFetchApp from Google's IP/UA, not the user's), so these explicit params are the only device signal; register them as custom dimensions in GA4 Admin. `user_id` is a **SHA-256 hash** of the email (pseudonymous, PDPA) via `hashUserId_()` + optional `GA4_USER_ID_SALT`. All best-effort — never throws, no-ops if unconfigured. Config in Script Properties (`GA4_MEASUREMENT_ID`, `GA4_API_SECRET`, `GA4_USER_ID_SALT`). `testTrackEvent()` is the editor-run harness (validates via `/debug/mp/collect`) |
...
- **Beneficiaries:** stored as JSON in `Beneficiary_Data` — `{name, rel, pct, address}`; max 5; **each pct ≥ 1** and they must sum to exactly 100; a **title prefix** + `name` + `rel` + `address` all required (validated in `validateStep`/`validateUpdateBen`). **Title prefix** (bank requirement): selected from a form dropdown (นาย/นาง/นางสาว/ด.ช./ด.ญ. + Mr./Ms./Mrs.); it's a *transient UI-only field* merged into `name` at submit via `mergePrefixes()` (so the stored model stays `{name, rel, pct, address}` and nothing downstream changed) — `splitPrefix()` re-separates a saved title back into the dropdown when editing, to avoid double-prefixing. `rel` stored as English key — picker offers Parent/Spouse/Child/Sibling/Relative/Friend (`Other` removed from the picker but kept in `REL_LABELS` for legacy display); displayed via `relLabel()`. Address required as of bank confirmation; the "Same as Above" checkbox copies the previous beneficiary's address.
